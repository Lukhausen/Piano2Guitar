...
itemsContainer.addEventListener('drop', handleDropOnItemList);
...
function createSelectedItemElement(item) {
    ...
    selectedItemElement.addEventListener('dblclick', removeSelectedItem);
...
function handleDropOnItemList(event) {
    event.preventDefault();
    const droppedItemId = event.dataTransfer.getData('text/plain');
    const droppedItemElement = document.getElementById(droppedItemId);

    // Check if the dropped item is a selected item
    if (droppedItemElement && droppedItemElement.classList.contains('selected-item')) {
        ...
        // Update the selectedItemsArray to remove the item
        selectedItemsArray = selectedItemsArray.filter(item => item !== droppedItemElement.textContent);
...
function handleDragEnd(event) {
    ...
    const overItems = document.querySelectorAll('.selected-item');
